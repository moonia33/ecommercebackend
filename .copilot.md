...
- **Checkout integracija (front-end):**
  ...
  - `shipping_method` = `lpexpress_courier` reiškia Unisend/LPExpress kurjerį (pickup point nereikia).
...
- **Admin terminalų sinchronizavimas:**
  ...
  - Sync pažymi `is_active=False` terminalus, kurių API nebegrąžina (tinka periodiniams atnaujinimams).

### Periodiniai atnaujinimai (management commands)

- **DPD paštomatai (lockers):**
  - `python manage.py dpd_sync_lockers --country-code LT --deactivate-missing`
- **Unisend/LPExpress terminalai:**
  - `python manage.py unisend_sync_terminals --country-code LT --deactivate-missing`

Šias komandas galima leisti per cron/Task Scheduler (pvz. 1x per dieną), kad atsiradę/perkelti/uždaryti paštomatai korektiškai atsinaujintų.

### Migracijos: svarbus workflow

- Kai **sukuri migracijas** (`python manage.py makemigrations`) arba **atsitrauki pakeitimus iš repo**, visada paleisk:
  - `python manage.py migrate`

Tai pritaiko DB schemos pakeitimus (modelius, laukus, seed migracijas ir t.t.).
